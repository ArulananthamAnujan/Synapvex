...
export default function AdminFAQs() {
  ...
  return (
    <DashboardLayout navItems={adminNavItems} title="FAQ Manager" subtitle="Manage frequently asked questions shown on the contact page">
      <div className="space-y-5 max-w-4xl">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={startNew}
            className="flex items-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white text-sm font-bold rounded-xl transition-colors"
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
            ...
            <p className="text-slate-500">No FAQs yet. Add your first one above.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {faqs.map(faq => (
              <div key={faq.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all ${!faq.is_active ? 'border-slate-100 opacity-60' : 'border-slate-100'}`}>
                {editing === faq.id ? (
                  <div className="p-6">
                    <FAQForm form={form} setForm={setForm} saving={saving} onSave={handleSave} onCancel={cancelEdit} />
                  </div>
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          placeholder="e.g. How do I enrol in a course?"
        />
      </div>
      <div>
        ...
        <div className="flex items-end pb-0.5">
          <label className="flex items-center gap-3 cursor-pointer">
            <div
              ...
              className={`relative w-10 h-6 rounded-full transition-colors cursor-pointer ${form.is_active ? 'bg-sky-600' : 'bg-slate-300'}`}
            >
      ...
      <div className="flex gap-3 pt-2">
        <button
          onClick={onSave}
          disabled={saving}
          className="flex items-center gap-2 px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-60"
        >
          <Save className="w-4 h-4" /> {saving ? 'Saving...' : 'Save FAQ'}
        </button>
        <button
          onClick={onCancel}
          className="flex items-center gap-2 px-5 py-2.5 border border-slate-200 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors"
        >
