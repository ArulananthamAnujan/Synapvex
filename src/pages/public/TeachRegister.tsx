...
import { AlertCircle, CheckCircle2, ArrowLeft, CheckCircle, Zap, Sparkles, ShieldCheck } from 'lucide-react';
...
export default function TeachRegister() {
  ...
  // `redirectTo` sends free signups to the dashboard and paid signups to
  // Plan & Billing, where they complete checkout with their new account.
  const startTeacherGoogle = async (redirectTo: string) => {
    setError('');
    setLoading(true);
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-sm">
            ...
            <button
              type="button"
              onClick={() => startTeacherGoogle('/dashboard')}
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3.5 bg-white border border-slate-300 rounded-xl text-[15px] font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-400 transition-colors shadow-sm disabled:opacity-60"
            >
              <GoogleG />
            ...
            <div className="mt-5 flex items-start gap-2.5 text-xs text-slate-400">
              ...
              <p>Secure sign-up with Google — no password needed. Your free AI credits are added automatically.</p>
            </div>
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <button
                type="button"
                onClick={() => startTeacherGoogle('/teacher/billing')}
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 px-4 py-3.5 bg-white border border-slate-300 rounded-xl text-[15px] font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-400 transition-colors shadow-sm disabled:opacity-60"
              >
                <GoogleG />
              ...
              <p className="text-center text-xs text-slate-400">
                We'll create your account with Google, then take you to checkout to start your {selectedPlan?.name ?? ''} plan.
              </p>
